...
- **v3.16** — Fix: whisper-ane модель загружалась (v3.15 фикс), но `transcribe()` зависал навсегда на коротких dictation-клипах (1–3 сек). Симптом: индикатор «Распознавание…» крутится бесконечно, текст никогда не вставляется, процесс спит (CPU 0%). Корень: я передавал `DecodingOptions(... chunkingStrategy: .vad)`. WhisperKit's VAD ждёт voice activity segment которого на коротком клипе нет, и `await transcribe` не возвращается. Решение: убрать `chunkingStrategy` совсем — для коротких клипов WhisperKit просто прогоняет encoder/decoder по всему буферу, что нам и нужно для Option+Space dictation.
